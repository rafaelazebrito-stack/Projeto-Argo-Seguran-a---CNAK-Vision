...
st.set_page_config(page_title="CNAK Vision - Smart Hub", page_icon="💠", layout="wide")
...
# ==============================================================================
# 2.5. EFEITOS DE INICIALIZAÇÃO (SPLASH SCREEN ARGOS SEGURANÇA)
# ==============================================================================
def initialization_splash_screen():
    """
    Exibe o splash screen com efeitos de movimentação e menção à Argos Segurança.
    O efeito é gerenciado por HTML/CSS/JS e só é exibido uma vez.
    """
    
    # Efeito de inicialização com HTML/CSS/JS (Movimentação e Branding)
    components.html(
        """
        <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700&family=Rajdhani:wght@300;500&display=swap');
        
        /* Overlay que cobre a tela durante a inicialização */
        #splash {
            position: fixed;
            top: 0; left: 0; right: 0; bottom: 0;
            background: linear-gradient(180deg, #0a0803 0%, #1a1204 100%);
            z-index: 9999;
            display: flex;
            ...
            align-items: center;
            opacity: 1;
            transition: opacity 2s ease-out; /* Transição de fade out */
        }
        ...
        /* Efeito Especial de Movimentação: Linha de Scanline (Movimentação Horizontal) */
        .scanline-container {
            width: 80%;
            max-width: 600px;
            height: 30px;
        ...
        .scanline {
            ...
            width: 100%;
            height: 4px;
            background: linear-gradient(to right, transparent, rgba(30, 200, 255, 0.9), transparent);
            transform: translateY(-50%) translateX(-100%);
            animation: scanLineMove 2s cubic-bezier(0.8, 0.1, 0.2, 0.9) infinite;
            box-shadow: 0 0 10px rgba(30, 200, 255, 0.9);
        }
        
        /* Animação do efeito de movimentação (scanline) */
        ...
        <div id="splash">
            <div class="splash-title">CNAK VISION HUB</div>
            <div class="scanline-container"><div class="scanline"></div></div>
            <div class="argos-tag">Tecnologia desenvolvida pela ARGOS SEGURANÇA</div>
        </div>

        <script>
            // Tempo total de exibição do splash: 2 segundos + 2 segundos de fade = 4 segundos
            setTimeout(() => {
                const splash = document.getElementById('splash');
                // Inicia o fade out após 2 segundos
                splash.style.opacity = '0';
                
                // Remove o elemento completamente após a transição para liberar a interação
                setTimeout(() => {
        ...
        """,
        height=50, # Altura mínima para o componente ser renderizado, embora o CSS o torne full screen
        key="initial_splash"
    )
# Fim da função do splash screen
# ==============================================================================
...
# NOVO: Variável de estado para controlar a exibição do splash screen
if 'splash_shown' not in st.session_state:
    st.session_state.splash_shown = False

if not st.session_state.logged_in:
    # Exibe o splash screen na primeira execução da sessão
    if not st.session_state.splash_shown:
        initialization_splash_screen()
